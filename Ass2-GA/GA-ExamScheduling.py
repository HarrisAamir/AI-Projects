...
def getStudentClashesCount(courseslist): #returns the count of students with clashes between the courses of courcelist
...
def fitness(chromosom):
    ...
    for i in range(N):
        if courses[i] not in coursesList:
            return 4000 #chromosom have reperting cource

    if len(chromosom)!=N:
        return 4000 # shows that chromosome is missing a cource 
    ...
    for hall in halls:
        ...
        for sol in chromosom:
            if sol[2] == hall:
                halltime+=int(getCourceTime(sol[0]))
